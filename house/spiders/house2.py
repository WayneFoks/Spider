...
class HouseSpider2(scrapy.Spider):
    ...
    def parse_house(self, response):
        ...
        yield {
            ...
            '小区': getValueSafely(extract_with_css('div.house-title > div::text').extract_first().split(' '), 0).strip(),
            '建筑面积': getValueSafely(
            ...
            '所在楼层': getValueSafely(
                response.css('div.introContent').css('div.content').css('li::text')[1].extract().split("("), 0).strip(),
            '总楼层': getValueSafely(response.css('div.introContent').css('div.content').css('li::text')[1].re('共(\d*)层'),
                                  0).strip(),
