...
class HouseSpider2(scrapy.Spider):
    ...
    def parse_house(self, response):
        ...
        yield {
            ...
            '挂牌价': extract_with_css('div.msg').css('span > label::text').extract()[0].strip(),
            ...
            '小区': extract_with_css('div.house-title > div::text').extract_first().split(' ')[0].strip(),
            '建筑面积': getValueSafely(
                response.css('div.introContent').css('div.content').css('li::text')[2].re('(\d*\.*\d*)'), 0).strip(),
            ...
            '所在楼层': response.css('div.introContent').css('div.content').css('li::text')[1].extract().split("(")[
                0].strip(),
            '总楼层': response.css('div.introContent').css('div.content').css('li::text')[1].extract().split("(")[
                1].strip(),
